...
@on_command('/签到帮助', False, command_type=[MessageType.room_chat, MessageType.private_chat])
async def show_help(Message):
    help_message = (r"\\\* "
        "欢迎使用签到机器人！以下是可用命令及其说明：\n\n"
        "**1. /z** - 签到命令，使用此命令可以签到并获得今日运势！\n"
        "签到后，机器人会返回签到信息，包括你的运势和一张美图。\n\n"
        "**2. /rank** - 查看签到排行榜，使用此命令可以查看今天的签到排名。\n\n"
        "**运势说明：**\n"
        "- 上上：最高等级，今日运势极佳！\n"
        "- 上吉：较高等级，运势良好！\n"
        "- 中吉：中等吉利，运势一般。\n"
    ...
    await API.send_msg(Message, help_message)

# 这样，用户可以通过发送 /help 来查看使用说明和运势说明。
def fetch_image() -> str:
